...
As mentioned previously, WKTUI lets you add a model to the project by discovering the model from an existing domain.  To
do this, WKTUI uses the WDT [Discover Domain Tool](https://oracle.github.io/weblogic-deploy-tooling/userguide/tools/discover/).  
...
1. `Collect artifacts from the local file system` - This requires that WKTUI is running on the same machine as the
    Admin Server.
...
3. `Use SSH to collect artifacts for remote domain` - This option relies on having SSH configured so that WKTUI can tell
    WDT to establish an SSH connection to the machine where the Admin Server is running so that it can collect the files.
    **Note that this depends on you having previously used SSH (outside the WKTUI application) to establish an SSH
...
The WKTUI QuickStart bundle includes scripts to help accomplish this goal.  Go to the directory where you expanded the
QuickStart ZIP file; we refer to this directory as `$WKTUI_QS_HOME` in the following instructions.  

Do the following steps:
1. In the `$WKTUI_QS_HOME` directory, edit the appropriate `setQuickstartEnv` script file (for example, 
...
7. Start the Administration Server of the newly created domain using the `$WKTUI_QS_HOME/todolist_domain/startWebLogic`
   script.
8. After the Administration Server is started, open another command-line window, change to the 
   `$WKTUI_QS_HOME/todolist_domain` directory, and run the appropriate `bin/startManagedWebLogic` script like this:
