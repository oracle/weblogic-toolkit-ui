...
define([],
  function () {
    ...
    class ShellScriptBase {
      ...
      addWitCacheCommandBlock(comment, imageToolScript, installerArchitecture, installerType, installerPath, installerVersion, errorMessage) {
        const args =
          `cache addInstaller --force --architecture=${installerArchitecture} --type=${installerType} --path="${installerPath}" --version=${installerVersion}`;
