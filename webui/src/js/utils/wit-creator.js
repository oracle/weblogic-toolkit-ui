  ...
  function WktImageCreator() {
    ...
    this.callCreateImage = async (options) => {
      ...
      if (this.project.settings.targetDomainLocation.value === 'pv' && !this.project.image.createCustomImageForPV.value) {
        const errMessage = i18n.t('wit-creator-domain-location-pv-message');
      ...
      try {
        ...
        if (this.usingWdt()) {
          ...
          if (runPrepareModelPromptResult) {
            ...
            if (!success) {
              wktLogger.error('Create Image failed because nested Prepare Model call returned a failure.');
              dialogHelper.closeBusyDialog();
              return Promise.resolve(false);
            } else {
              wktLogger.info('Create Image continuing after nested Prepare Model call returned success.');
            }
          }
    ...
    this.getValidatableObject = (flowNameKey) => {
      ...
      validationObject.addField('image-design-image-tag-label',
        this.project.image.imageTag.validate(true));
      if (this.requiresInstaller('javaHome')) {
        validationObject.addField('image-design-jdk-installer-title',
          validationHelper.validateRequiredField(this.project.image.jdkInstaller.value));
        validationObject.addField('image-design-jdk-installer-version-label',
          validationHelper.validateRequiredField(this.project.image.jdkInstallerVersion.value));
      }
      if (this.requiresInstaller('oracleHome')) {
        validationObject.addField('image-design-fmw-installer-title',
          validationHelper.validateRequiredField(this.project.image.oracleInstaller.value));
        validationObject.addField('image-design-fmw-installer-type-label',
          validationHelper.validateRequiredField(this.project.image.oracleInstallerType.value));
        validationObject.addField('image-design-fmw-installer-version-label',
          validationHelper.validateRequiredField(this.project.image.oracleInstallerVersion.value));
      }
      if (this.requiresInstaller('wdtHome')) {
        if (!this.project.image.useLatestWdtVersion.value) {
          validationObject.addField('image-design-wdt-installer-label',
            validationHelper.validateRequiredField(this.project.image.wdtInstaller.value));
          validationObject.addField('image-design-wdt-installer-version-label',
            validationHelper.validateRequiredField(this.project.image.wdtInstallerVersion.value));
        }
      }
      ...
      if (this.supportsPatching()) {
        if (this.project.image.applyOraclePatches.value) {
          validationObject.addField('image-design-support-username-label',
            validationHelper.validateRequiredField(this.project.image.oracleSupportUserName.value));
          validationObject.addField('image-design-support-password-label',
            validationHelper.validateRequiredField(this.project.image.oracleSupportPassword.value));
        }
      }
    ...
    this.buildCreateConfigObject = (projectDirectory, javaHome, imageBuilderExe, jdkInstallerVersion,
      ...
      const createConfig = {
        ...
        chownOwner: this.project.image.fileOwner.value,
        chownGroup: this.project.image.fileGroup.value,
        alwaysPullBaseImage: this.project.image.alwaysPullBaseImage.value,
      };

      if (this.project.image.useCustomBaseImage) {
    ...
    this.requiresInstaller = (type) => {
      let result = true;
      if (type === 'wdtHome') {
        result = this.project.settings.targetDomainLocation.value !== 'pv';
