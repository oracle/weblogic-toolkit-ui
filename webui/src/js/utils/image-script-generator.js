...
define(['models/wkt-project', 'utils/script-generator-base'],
  function(project, ScriptGeneratorBase) {
    const scriptDescription = [
      'This script uses the WebLogic Image Tool to generate a new image that will be',
    ...
    class ImageScriptGenerator extends ScriptGeneratorBase {
      ...
      generate() {
        ...
        const usingWdt = this.project.settings.targetDomainLocation.value !== 'pv';
        ...
        if (this.project.settings.targetDomainLocation.value === 'pv' && !this.project.k8sDomain.createCustomImageForPV.value) {
          this.adapter.addEchoLine('Using domain from persistent volume without building a custom image...nothing to do');
        ...
        if (usingWdt) {
          ...
          if (this.project.image.modelHomePath.hasValue()) {
            modelHomeValue = this.project.image.modelHomePath.value;
          }
          this.adapter.addVariableDefinition('WDT_MODEL_HOME', modelHomeValue);
        ...
        if (usingWdt) {
          ...
          this.adapter.addWitCreateWdtArgs('WIT_CREATE_ARGS', 'Gather WDT-related arguments.',
            wdtVersion, wdtTargetType, wdtDomainType, wdtDomainHome, wdtModelHome, modelFile, variableFile, archiveFile);
        }
