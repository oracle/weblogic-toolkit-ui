...
define(['utils/i18n', 'accUtils', 'knockout', 'ojs/ojcorerouter', 'ojs/ojmodulerouter-adapter',
  'ojs/ojarraydataprovider', 'utils/wko-installer', 'utils/wko-updater', 'utils/wko-uninstaller'],
function(i18n, accUtils, ko, CoreRouter, ModuleRouterAdapter, ArrayDataProvider, wkoInstaller, wkoUpdater, wkoUninstaller) {
  function OperatorViewModel(args) {
    ...
    this.disableUpdateOperator = ko.observable(false);
    this.disableUninstallOperator = ko.observable(false);

    this.installOperator = async () => {
      await wkoInstaller.startInstallOperator();
    };

    this.updateOperator = async () => {
      await wkoUpdater.startUpdateOperator();
    };

    this.uninstallOperator = async () => {
      await wkoUninstaller.startUninstallOperator();
    };
