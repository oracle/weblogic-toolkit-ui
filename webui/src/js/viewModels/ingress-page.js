...
define(['accUtils', 'knockout', 'ojs/ojcorerouter', 'ojs/ojmodulerouter-adapter', 'ojs/ojarraydataprovider',
  'utils/i18n', 'utils/ingress-controller-installer', 'utils/ingress-routes-updater',
  'utils/ingress-controller-uninstaller'
],
function(accUtils, ko, CoreRouter, ModuleRouterAdapter, ArrayDataProvider, i18n,
  ingressControllerInstaller, ingressRouteUpdater, ingressControllerUninstaller) {
  function IngressViewModel(args) {
    ...
    this.disableUninstallIngress = ko.observable(false);
    ...
    this.updateRouting = async () => {
      await ingressRouteUpdater.startIngressRoutesUpdate();
    };

    this.uninstallIngress = async () => {
      await ingressControllerUninstaller.startUninstallIngressController();
    };
